...
class NASimNetGNN_LSTM(Net):
    def __init__(self):
        super().__init__()

        self.embed_node = Sequential( Linear(config.node_dim, config.emb_dim), LeakyReLU() )
        self.gnn = MultiMessagePassingWithGlobalNodeAndLastGRUGlobal(config.mp_iterations)
    ...
    def update(self, trace, target_net=None, hidden_s0=None):
        ...
        # DISABLED (this won't work anymore with recurrency):
