...
def get_args(problem_config):
	...
	parser.add_argument('-batch', type=int, default=256, help="Number of parallel environments")
	...
	parser.add_argument('-alpha_h', type=float, default=0.2, help="Initial entropy regularization constant")
...
if __name__ == '__main__':
	...
	if args.eval:
		r_avg, s_ps_avg, s_avg, s_tot = problem_debug.evaluate(net)
	...
	for step in itertools.count(start=1):
		...
		for step_trace in range(config.ppo_t):
			...
			a, v, q, pi, raw_a = net(s)
			s, r, d, i = env.step(a)

			a_q = [0 if a_action == -1 else 1 for (a_node, a_action) in a] # action_q - 0 = terminate / 1 = continue

			s_true = [x['s_true'] for x in i]
			d_true = [x['d_true'] for x in i]

			trace.append( (s_orig, raw_a, a_q, r, s_true, d_true) )

		# update network
		# loss, entropy, norm, pi_deviations = net.update(s_orig, raw_a, r, s_true, d_true, target_net)
		loss, entropy, norm, pi_deviations = net.update(trace, target_net)
